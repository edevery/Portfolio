...
void main() {
    ...
    // Base color: #85c3ed
    vec3 baseColor = vec3(0.522, 0.765, 0.929);

    // Animated gradient with color variation
    ...
    // Vary brightness and slightly shift hue
    float brightness = 0.85 + combinedWave * 0.15;
    vec3 color = baseColor * brightness;

    // Add slight warmth variation based on mouse proximity
    color = mix(color, color * vec3(1.1, 1.0, 0.95), mouseInfluence * 0.3);

    gl_FragColor = vec4(color, stroke);
}
`;

interface EmilyMsdfCanvasV3Props {
  className?: string;
  circleSize?: number;
  circleEdge?: number;
  borderSize?: number;
...
const EmilyMsdfCanvasV3: FC<EmilyMsdfCanvasV3Props> = ({
  className = "",
  circleSize = 0.35,
  circleEdge = 0.55,
  borderSize = 0.45,
...
export default EmilyMsdfCanvasV3;
